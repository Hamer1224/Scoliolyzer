# scoliosis_analyzer_ctk_fixed.py
# Düzeltilmiş versiyon: axis-aligned kutularla çalışan, komşu-centre tabanlı lokal eğim hesaplaması.
import cv2
import numpy as np
import math
...
class ScoliosisAnalyzer:
    ...
    def __init__(self, master):
        self.master = master
        self.master.title("Scoliosis Analyzer v0.2 - Fixed")
        ...
        try:
            # Model dosyanızı burada ayarlayın:
    ...
    def assign_anatomical_labels(self, vertebrae):
        # Basit: en alttan en üste doğru sıralayıp etiket atar.
        labels = ["S1"] + [f"L{i}" for i in range(5, 0, -1)] + [f"T{i}" for i in range(12, 0, -1)] + [f"C{i}" for i in range(7, 0, -1)]
        vertebrae.sort(key=lambda v: v["center"][1], reverse=True)  # en alttan en üste
    ...
    def find_cobb_angle(self, vertebrae, image):
        try:
            ...
            # 1. Komşu-centre tabanlı lokal eğimler
            tangent_angles = self.get_local_tangent_angles(vertebrae, neighbor_span=1)

            # 2. Eğimin (tilt) aralığını kontrol et
            ...
            # 3. Maksimum ve minimum tiltli vertebrayı bul
            v_upper_idx = int(np.argmin(tangent_angles))  # üst end için daha negatif (sola eğim) olabilir
            v_lower_idx = int(np.argmax(tangent_angles))

            # 4. Hangi vertebra üst / alt olduğunu y merkezine göre doğrula
            if vertebrae[v_upper_idx]['center'][1] > vertebrae[v_lower_idx]['center'][1]:
                # swap: üst indeksi daha küçük y olmalı (y arttıkça daha altta)
            ...
            # 5. Cobb açısını hesapla (iki local tangent açısı arasındaki fark)
            ...
            # 6. Çizimleri yap (line'ları center ve açı ile çiz)
    ...
    def draw_line_through_center_with_angle(self, img, center, angle_deg, color, thickness, label=None):
        ...
        """
        cx, cy = center
        # açıdan doğrusal eğimi hesapla
        angle_rad = math.radians(angle_deg)
        # Çok dik ise (tan büyük) dik çizgi çiz
